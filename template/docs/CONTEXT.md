...
### 角色边界 = 会话边界
5 个 AI 角色（设计辅助者 / 计划拆解者 / 实施者 / 审查者 / 文档维护者）原则上应映射到独立 session；不得由单 session 串完全部角色。

**例外**：当某一角色的判断被 ADR / 单点文件严格约束时，可与相邻 session 合并。当前实例：

- 实施者 + 文档维护者 = 1 session（合并依据：[doc-rewriting-rules.md](ai/doc-rewriting-rules.md) 的 4 条触发条件 + 评审 session 兜底）

详见 [ai-role-boundaries.md](ai/ai-role-boundaries.md)。
...
### Adoption Profile（用户项目元信息）
目标项目接入本脚手架时必须在 `AGENTS.md` 填写的项目画像，包含包管理器、主要应用目录、入口代码锚点、共享包目录、测试目录和完整验证入口。它是 AI 定位锚点与验证入口的单一来源；`context-index.md` 和 doctor 只能引用或检查它，不维护第二份项目元信息。

> 模板（含 5 个 `<...>` 占位符与填写方式说明）在 [`template/AGENTS.md`](../AGENTS.md)；仓库根 `AGENTS.md` 是本仓库专用入口（已按脚手架自身事实填好，复制 `template/AGENTS.md` 后填入 Adoption Profile）。
