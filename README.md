...
2. **必须定义 `verify` 命令**——在你项目的 manifest 中定义一个 `verify` 入口，串联 lint → typecheck → test → build；L1+ 任务完成前 AI 必跑（详见 [ADR-0002](template/docs/adr/0002-verify-hard-gate.md)）
3. **运行接入自检**——执行 `bash template/scripts/scaffold-doctor.sh`（等价于 `--adopted`），并按 [adoption-checklist.md](template/docs/ai/checklists/adoption-checklist.md) 处理 `FAIL` / `WARN`；doctor 只检查脚手架接入状态，不替代项目 `verify`
4. **跑一次 L0 任务试水**——用本文档试一次小改动，跑一次最小验证
5. **跑一次 L2 任务验证流程**——按 [l2-multi-session-runbook.md](template/docs/ai/runbooks/l2-multi-session-runbook.md) + [feature-delivery-runbook.md](template/docs/ai/runbooks/feature-delivery-runbook.md) 跑通一次新功能（3 session 串行：规划 → 实施 → 评审；spec + plan 物理分离，验证证据统一落点到两份文件）

> **已取代**：本节原表述为"4 session 串行：设计 → 计划 → 实施 → 评审"——该 4 Session 措辞将 L2 与 L3 合并为同一模型，与 [ADR-0003](template/docs/adr/0003-multi-session-l2.md) 2026-08-01 修订（**L2 = 三 Session；L3 = 四 Session**）不相容。现行 L2 流程以本节首段为准。

## 核心治理机制

- **任务分级 L0/L1/L2/L3**：见 [template/docs/ai/task-levels.md](template/docs/ai/task-levels.md)
...
- **完成定义 5 条件**：见 [template/docs/ai/completion-criteria.md](template/docs/ai/completion-criteria.md)
- **验证基线分层**：见 [template/docs/ai/verification-baseline.md](template/docs/ai/verification-baseline.md)（L1+ AI 必跑 verify）
- **分支与 worktree 策略**：见 [template/docs/ai/branch-strategy.md](template/docs/ai/branch-strategy.md)
- **AI 角色边界**：见 [template/docs/ai/ai-role-boundaries.md](template/docs/ai/ai-role-boundaries.md)（L2+ 角色边界=会话边界）
- **文档回写规则**：见 [template/docs/ai/doc-rewriting-rules.md](template/docs/ai/doc-rewriting-rules.md)
- **仓库术语表**：[template/docs/CONTEXT.md](template/docs/CONTEXT.md)
- **硬约束 ADR**：
  - [ADR-0002 verify 硬门禁](template/docs/adr/0002-verify-hard-gate.md)
  - [ADR-0003 L2 三 Session / L3 四 Session](template/docs/adr/0003-multi-session-l2.md)
  - [ADR-0004 L2 spec + plan](template/docs/adr/0004-l2-spec-and-plan.md)
  - [ADR-0005 L3 审批门禁](template/docs/adr/0005-l3-approval-gate.md)

## 模板与清单

- 任务包（L1）：[template/docs/ai/templates/task-packet.md](template/docs/ai/templates/task-packet.md)
- 功能设计（L2 spec）：[template/docs/ai/templates/feature-spec.md](template/docs/ai/templates/feature-spec.md)
- 实施计划（L2 plan）：[template/docs/ai/templates/implementation-plan.md](template/docs/ai/templates/implementation-plan.md)
- 缺陷修复：[template/docs/ai/templates/bugfix-brief.md](template/docs/ai/templates/bugfix-brief.md)
- 重构：[template/docs/ai/templates/refactor-brief.md](template/docs/ai/templates/refactor-brief.md)
- 接入自检清单：[template/docs/ai/checklists/adoption-checklist.md](template/docs/ai/checklists/adoption-checklist.md)
- 评审清单：[template/docs/ai/checklists/review-checklist.md](template/docs/ai/checklists/review-checklist.md)
- ADR 模板：[template/docs/adr/adr-template.md](template/docs/adr/adr-template.md)
...
本脚手架不提供任何 AI 工具专属配置。L3 审批、verify 必跑和多 session 串行等约束都通过 `AGENTS.md`、`template/docs/ai/` 与 `template/docs/adr/` 表达；具体工具的 hook、rule 或插件只能作为项目自行添加的可选加固层。

## CI（验证层，非准入层）

- GitLab CI：[template/.gitlab-ci.yml](template/.gitlab-ci.yml)
- GitHub Actions：[template/.github/workflows/ci.yml](template/.github/workflows/ci.yml)

CI 跑 5 个 job：`lint-shell` / `lint-python` / `check-links` / `check-governance` / `check-doctor`。零第三方依赖，Python 3 标准库 + POSIX bash 即可执行。

**CI 是验证层，不是准入层**——CI 跑通只证明脚手架自身检查器与脚本在干净环境下能跑通，**不**等价于 L3 Pre-Implementation Approval Gate（详见 [ADR-0005](template/docs/adr/0005-l3-approval-gate.md)，那是准入层）。CI 结果可作为 L1+ 任务 verify 报告的事实证据（详见 [ADR-0002](template/docs/adr/0002-verify-hard-gate.md)）。
...
`--template` 模式适用于**本脚手架仓库自身**（仓库根 `AGENTS.md` 的 Adoption Profile 已按本仓库事实填好）。接入本脚手架的**目标项目**跑默认模式即可（`bash template/scripts/scaffold-doctor.sh`，等价于 `--adopted`，会额外检查 4 档验证入口是否映射到 manifest）。两种模式的差异见 [CONTRIBUTING.md](CONTRIBUTING.md) 与 doctor 顶部的 `--adopted|--template` 说明。
...
- 触及长期边界、默认做法、跨工具兼容性时，按 [template/docs/ai/doc-rewriting-rules.md](template/docs/ai/doc-rewriting-rules.md) 回写文档
- 新增单点定义时，把已有的重复引用全部改链到新单点
- 维护模板仓库时使用 `bash template/scripts/scaffold-doctor.sh --template`；接入目标项目时使用默认 `bash template/scripts/scaffold-doctor.sh`
