...
- 没有工具无关机制限制 L3 范围内的高风险动作
- 可选工具适配层还没有被定义为本脚手架的核心能力
...
2. **批准信号必须引用具体 spec/plan 双份路径**：批准消息中必须**同时**出现 `docs/specs/<date>-<name>.md` **与** `docs/plans/<date>-<name>.md` 的完整路径；任一份路径缺失视为"未批准"
...
8. **批准记录（spec / plan 中的 `## 批准` 段）的最小必含项**：
   - **批准信号**：批准消息原文或可追溯引用
   - **spec 路径**：`[docs/specs/<date>-<name>.md](...)`（与 spec 文件名完全一致）
   - **plan 路径**：`[docs/plans/<date>-<name>.md](...)`（与 plan 文件名完全一致）
   - **允许修改范围**：与 spec 中 `## 目标` / `## 行为` / `## 非目标` 段对齐的具体可操作范围（可被外部判断）
   - **禁止范围**：明确列出本次不批准的事项（新依赖 / 新文件 / 新接口 / 重命名 / 配置变更等）
   - **批准时间**：YYYY-MM-DD
   任一项缺失，AI 必须视为"未批准"，不得开始实施 session。
9. **批准不得跨任务复用**：每个新 L3 任务必须重新走"spec + plan + 批准"全流程；任何沿用过往任务批准的"延续批准"视为未批准。即便两次任务改动落入同一文件、同一区域，仍需独立 spec / plan / 批准三件套。
...
- 约束或成本：
  ...
  - 用户的"批准动作"成本：必须写出明确的"已批准"字眼、同时引用 spec 与 plan 双份路径、并明确"允许修改范围 / 禁止范围"
  - 实施 session 期间若发现"超出范围"需要回到批准环节；增加中途回退的可能
  - 隐含同意（如"👌"）不被接受；用户必须切换到显式语言
  - **批准跨任务不复用**：每个新任务的 spec / plan / 批准三件套必须独立生成；模板维护与评审方都能识别"该批准仅约束当次任务"
  - 模板层面：spec 与 plan 必须分别在 `## 批准` 段落齐第 8 项最小必含；任一项缺失视为未批准
- 后续触发条件：
  - 若后续引入工具无关的高风险动作检查，并对 `git add` / `git commit` 增加 L3 spec 路径匹配检查，本 ADR 的"AI 缺信号不得 commit"可由外部机制强制而非依赖 AI 自律
...
- 模板：[../ai/templates/feature-spec.md](../ai/templates/feature-spec.md)、[../ai/templates/implementation-plan.md](../ai/templates/implementation-plan.md)（`## 批准` 段均含第 8 项最小必含；plan 中 `## 批准` 顺序在 `## 验证证据` 之前）

> **已取代**：本 ADR 早前版本对"批准记录"只点名字段（时间 / 来源 / 范围），未列出最小必含项；早前措辞"批准信号必须引用具体 spec/plan 路径"与 [ADR-0004](0004-l2-spec-and-plan.md) 的"spec + plan 双份"不完全对齐（仅引用单份即视为合规）。本 ADR 的第 8 项（最小必含）、第 9 项（不跨任务复用）2026-08-01 起取代上述宽泛表达——**L3 批准记录必须包含：批准信号、spec 路径、plan 路径、允许修改范围、禁止范围；批准不得跨任务复用**。
