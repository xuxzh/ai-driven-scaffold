...
L2+ 任务按 [ADR-0003](../adr/0003-multi-session-l2.md) 串行 4 个 session。每个 session 的阅读入口**按角色**分流——会话边界 = 角色边界，详见 [`docs/ai/ai-role-boundaries.md`](./ai-role-boundaries.md)：
...
#### 评审 session（审查者，**默认新开 session**）
1. `AGENTS.md`（边界）
2. 上一 session 交付物：`git diff <base>..HEAD` + spec + plan + `## 验证证据` 段
3. `docs/ai/checklists/review-checklist.md`（评审清单）
4. `docs/adr/0003-multi-session-l2.md`（评审独立性 + session 默认新开）
5. `docs/adr/0002-verify-hard-gate.md`（verify 必跑纪律检查）
6. **不**预读实施 session 的中间对话
7. review report **必含**：测试盲区清单 + 未跑项确认（详见 [`docs/CONTEXT.md`](../CONTEXT.md)）

L3 任务的实施 session 在启动前还需读 [ADR-0005](../adr/0005-l3-approval-gate.md) 并等待用户"已批准"信号。
...
| `L2` 小任务（< 半天，快速通道例外） | 同上 + spec 顶部加 `## 快速通道` 段 | 合并"设计 + 计划"为 1 session（仍保留"实施 + 评审"分离）；**不豁免 spec**，只豁免 spec 与 plan 的物理分离 |
